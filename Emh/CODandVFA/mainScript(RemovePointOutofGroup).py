...
for index in range(steps):
    ...
    title = 'Relative error (GBR for VFA out, Single Day, Removed)\nMAE: ' + (str(MeanErrorRate))[:6] + '; R2: ' + str(R2)[:5]
    ...
    # plt.savefig("./RemoveErrorSingleDayGBR4VFA" + str(index) + ".png", dpi=100)
    plt.show()
    ...
    title = 'Relative error(SVR for VFA out, Single Day, Removed)\nMAE: ' + (str(MeanErrorRate))[:6] + '; R2: ' + str(R2)[:5]
    plt.figure(figsize=(18, 12), dpi=100)
    # plt.scatter(y_fixU, y_valueU, s=markerSizeU, color=color, marker='+', label='classifier predict(predict to low)')
    # plt.scatter(y_fixD, y_valueD, s=markerSizeD, color=color, marker='x', label='classifier predict(predict to high)')
    # plt.scatter([i for i in range(len(pe))], pe, color='darkorange', label='gap')
    ...
    # plt.savefig("./RemoveErrorSingleDaySVR4VFA" + str(index) + ".png", dpi=100)
    plt.show()
...
# draw
data.boxplot()
plt.ylabel("real VFA out")
plt.title("box of real VFA out")
plt.show()"""
