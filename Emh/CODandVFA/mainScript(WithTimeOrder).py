...
for it in range(1, nRows-1):
    try:
        if table.cell_value(it+1, 0)-table.cell_value(it, 0) == 1:
...
for index in range(5):
    ...
    title = 'GBR for VFA out (2 Consecutive Day)\nMAE: ' + (str(MeanErrorRate))[:6] + '; R2: ' + str(R2)[:5]
    ...
    plt.xlabel('samples')
    plt.ylabel('VFA Out')
    plt.savefig("./Result/2 Consecutive Day GBR4VFA "+str(index)+".png", dpi=300)
    # plt.show()
    ...
    title = 'SVR for VFA out (2 Consecutive Day)\nMAE: ' + (str(MeanErrorRate))[:6] + '; R2: ' + str(R2)[:5]
    plt.figure(figsize=(18, 12), dpi=300)
    # plt.scatter(y_fixU, y_valueU, s=markerSizeU, color=color, marker='+', label='classifier predict(predict to low)')
    # plt.scatter(y_fixD, y_valueD, s=markerSizeD, color=color, marker='x', label='classifier predict(predict to high)')
    plt.plot([i for i in range(len(error))], error, color='green', label='gap')
    ...
    plt.xlabel('samples')
    plt.ylabel('VFA Out')
    plt.savefig("./Result/2 Consecutive Day SVR4VFA " + str(index) + ".png", dpi=300)
    # plt.show()
