...
def classifier(target):
    ...
    wb = xlrd.open_workbook('./errorCounterFix.xls')
    ...
    for it in range(1, ws.nrows):
        try:
            rX.append([float(ws.cell_value(it, 2)), float(ws.cell_value(it, 3)), float(ws.cell_value(it, 4)),
            ...
            pass
    print('%s, %s, %s' % (c_1, c1, c0))
    print((c_1 + c1) / (c_1 + c1 + c0))
